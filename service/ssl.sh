...
# 可选参数正则表达式
readonly OP_REGEX='(^--(help|update|purge|issue|(stop-)?renew|check-cron|status|info|www|domain|email|nginx|webroot|tls)$)|(^-[upirscdenwt]$)'
...
function show_certificate_info() {
    ...
    "${HOME}/.acme.sh/acme.sh" --info $(printf -- " -d %s" "${domains[@]}")
}
